...
describe("SalesDoNotRelyOnBrandRule", () => {
    it("should fail if sales are missing", () => {
    ...
    it("should fail if individual sales are missing", () => {
        const sales: any = [null];
        return SalesDoNotRelyOnBrandRule("some-id").guard(sales)
            .then(() => Promise.reject("Expected failure"))
            .catch(error => error.should.deep.equal(['"$[0]" was missing']));
    });

    it("should fail if individual sales are not objects", () => {
        const sales: any = [42];
        return SalesDoNotRelyOnBrandRule("some-id").guard(sales)
            .then(() => Promise.reject("Expected failure"))
            .catch(error => error.should.deep.equal(['"$[0]" was not an object']));
    });

    it("should fail if individual sales are missing brand", () => {
        const sales: any = [{}];
        return SalesDoNotRelyOnBrandRule("some-id").guard(sales)
            .then(() => Promise.reject("Expected failure"))
            .catch(error => error.should.deep.equal(['"$[0].brand" was missing']));
    });
